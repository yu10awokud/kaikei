...
4. **Framework Preset を確認する**
   自動で `Next.js` と表示されます。
   **Root Directory・ビルドコマンドは触らなくて大丈夫です**
   （アプリがリポジトリ直下にあるため、Vercel が自動で見つけます）。
...
> リポジトリ直下の `expense-app/` フォルダは、別で作った「部費立替メモ」アプリです。
> Vercel の別プロジェクトとして公開するもので、このアプリとは無関係なので触らなくて大丈夫です。
